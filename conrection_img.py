...
class ImgCorrect():
    ...
    def img_lines(self):
        ...
        self.lines = cv2.HoughLinesP(edges, 1, np.pi / 180, 150, minLineLength=100, maxLineGap=100)
        ...
        for x1, y1, x2, y2 in lines1[:]:
            cv2.line(imglines, (x1, y1), (x2, y2), (255, 0, 0), 1)
        cv2.imshow('lines', imglines)
        cv2.imwrite("aa1.jpg", imglines)
        return imglines
    ...
    def rotate_image(self, degree, saved_path):
        """
        正角 逆时针旋转
        :param degree:
        :return:
        ...
        cv2.imwrite(saved_path, imgRotation)
        return imgRotation
...
def load_img_path(images_path):
    if os.path.isdir(images_path):
        ...
        for i in file_list:
            if len(i[2]) > 0:
                for name in i[2]:
                    if name.__contains__('.JPG') and not name.__contains__('._'):
                        file_path_list.append(os.path.join(i[0], name))
        # print(images_path + '  目录下包含：{}  张图片/文件'.format(len(file_path_list)))
        file_path_list.sort()
...
if __name__ == "__main__":
    file_path_list, number_images = load_img_path(
        r'C:\Users\Desktop\result_imgs')
    file_path_save = r'C:\Users\Desktop\saveed'
    for index, image_path in enumerate(file_path_list[:]):
        # image_path = r'C:\Users\HuiKang\Desktop\t1\5555.jpg'

        path, name = os.path.split(image_path)
        image0 = cv2.imread(image_path)

        imgcorrect = ImgCorrect(image0)
        cv2.imshow("normalization image", imgcorrect.img)
        lines_img = imgcorrect.img_lines()

        saved_path = os.path.join(file_path_save, name)
        ...
        # cv2.imshow("lines_img",lines_img)
            ...
            imgcorrect.rotate_image(degree, saved_path)
    cv2.waitKey()
